...
const PostTitle = (props: Props) => {
    ...
    return (
        <div className='rounded-md bg-white h-fit w-full'>
            ...
            <div>
                <div className='flex p-5 pl-10 pb-10'>
                    {selectedTags.length > 0 && (
                        <div className='flex gap-4 py-2'>
                            {selectedTags.map((tags) => (
                                <p className='bg-pink-500 text-pink-600 bg-opacity-20 px-1 h-fit rounded-md text-lg cursor-pointer flex'>{tags}
                                    <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="15" height="15" viewBox="0 0 24 24" fill='#E91E63' className='size-0 hover:size-4'>
                    ...
                    <input className={`outline-none h-14 w-4/6 p-5 pl-10 pb-10 text-base rounded-md bg-transparent ${poppins_sm.className}`} onChange={(e) => setSearchTag(e.target.value)} type="text" placeholder='Add tags...' />
                </div>
                {tags.length > 0 && (
